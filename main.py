...
while True:
    try:
        ...
        logger.info(f"Input usuario: {prompt}", extra={"extra_data": {"user_prompt": prompt}})
        
        call_data = ask_orion(prompt)
        
        if call_data['CALL']:
            logger.info(f"Ejecutando: {call_data['CALL']}", extra={"extra_data": {"call": call_data}})
            result = dispatch(call_data['CALL'], call_data['ARGS'])
            print("\n[ORION]:\n", result)
            logger.info("Ejecución exitosa")
        else:
            print("\n[ORION]: No se pudo interpretar la instrucción.")
            logger.warning("No se pudo interpretar instrucción", extra={"extra_data": {"prompt": prompt}})
