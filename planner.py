...
class HybridTaskPlanner:
    ...
    def plan_task(self, user_input: str, context: dict) -> list:
        ...
        # 3. Fallback: No se pudo planificar, devolver None para ejecución normal
    ...
    def _rule_based_plan(self, user_input: str) -> list:
        ...
        if "creá proyecto web" in prompt_lower or "crear proyecto web" in prompt_lower:
            return [
                {"CALL": "create_folder", "ARGS": {"path": "proyecto_web"}},
                {"CALL": "create_file", "ARGS": {"path": "proyecto_web/index.html", "content": "<html><body><h1>Hola Mundo</h1></body></html>"}},
                {"CALL": "create_file", "ARGS": {"path": "proyecto_web/style.css", "content": "body { background-color: #f0f0f0; }"}}
            ]

        # Regla 2: "migrá proyecto" (Simple)
        if "migrá proyecto" in prompt_lower and "python" not in prompt_lower:
             return [
                {"CALL": "analyze_data", "ARGS": {"input_path": ".", "output_path": "migration_report.json"}},
                {"CALL": "create_file", "ARGS": {"path": "requirements_updated.txt", "content": "# Updated requirements"}}
            ]

        # Regla 3: "carpeta 'X' y archivo 'Y'" (Regex)
        # Match: creá carpeta 'foo' y archivo 'bar.txt'
        match_folder_file = re.search(r"carpeta ['\"](.+?)['\"] y archivo ['\"](.+?)['\"]", prompt_lower)
        if match_folder_file:
            folder = match_folder_file.group(1)
            file = match_folder_file.group(2)
            return [
                {"CALL": "create_folder", "ARGS": {"path": folder}},
                {"CALL": "create_file", "ARGS": {"path": f"{folder}/{file}", "content": f"Contenido de {file}"}}
            ]

        # Regla 4: "migrá proyecto de Python X a Y"
        match_migration = re.search(r"migr[áa] proyecto de python (.+?) a (.+)", prompt_lower)
        if match_migration:
            ver_old = match_migration.group(1)
            ver_new = match_migration.group(2)
            return [
                {"CALL": "analyze_data", "ARGS": {"input_path": ".", "output_path": f"migration_{ver_old}_to_{ver_new}.json"}},
                {"CALL": "create_file", "ARGS": {"path": "requirements.txt", "content": f"# Migrated to Python {ver_new}"}}
            ]
        ...
        if "backup de archivos" in prompt_lower:
            return [
                ...
                {"CALL": "list_files", "ARGS": {"path": "."}}, # Simulado: en realidad debería copiar
                {"CALL": "create_file", "ARGS": {"path": "backup/log.txt", "content": "Backup completado"}}
            ]
